...
def get_train_loader(dataset, height, width, batch_size, workers,
    ...
    train_loader = IterLoader(
        DataLoader(Preprocessor(train_set, root=None,
...
def main_worker(args):
    ...
    dataset_validation = get_data(args.dataset_validation, args.data_dir)
    test_loader_target = get_test_loader(dataset_validation, args.height, args.width, args.batch_size, args.workers)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1):
            mAP_1 = evaluator_1_ema.evaluate(test_loader_target, dataset_validation.query, dataset_validation.gallery,
                                             cmc_flag=False)
            mAP_2 = evaluator_2_ema.evaluate(test_loader_target, dataset_validation.query, dataset_validation.gallery,
                                             cmc_flag=False)
    ...
    evaluator_1_ema.evaluate(test_loader_target, dataset_validation.query, dataset_validation.gallery, cmc_flag=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-va', '--dataset-validation', type=str, default='target_validation',
                        choices=datasets.names())
